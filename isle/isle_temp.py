...
class RC2(object):
    ...
    def compute(self):
        ...
        debug = False
        # or_model, or_model_obj,  or_model_ub, status = solve_ortools(self.formula, self.forest, to = 600)

        # if not (or_model is None):
        #     print("ORMODEL")
        #     print("STATUS", status)
        #     print("ObjectiveValue ", or_model_obj)
        #     print("BestObjectiveBound ", or_model_ub)
        #     for var, val in or_model.items():
        #         print(f"{var} {int(val)}")
    ...
    def reactivate(self, model, rebuild = 0):
        if (self.circuitinject == CIRCUITINJECT_DELAYED):
            violated= 0
            for i in range(2**rebuild):
                vo = self.delayed_reactivation(model)
                violated += vo
                print(f"rebuild  {rebuild} +violated {vo}")
                if vo == 0:
                    break 
            return violated
        else:
