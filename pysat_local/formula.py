...
class IDPool(object):
    ...
    def occupy(self, start, stop):
        """
            ...
            :type start: int
            :type stop: intpython3 -u  ./res/res.py -c b  --ilp=360  -r mr1d /home/nina/workspace/data/mse21_complete_wt/tcp/tcp_students_91_it_15.wcnf.gz
...
class WCNF(object):
    ...
    def __init__(self, from_file=None, from_fp=None, from_string=None, from_gmodel = None,
            comment_lead=['c']):
        """
            Constructor.
        """
        ...
        if from_file:
            ...
            self.from_gmodel(from_gmodel, comment_lead)
    ...
    def from_gmodel(self, gmodel, comment_lead=['c']):
        """
        ...
        obj = gmodel.getObjective()
        obj_coeffs = [obj.getCoeff(i) for i in range(obj.size())]
        #print(obj, obj_coeffs)
        
        self.topw = int(max([abs(a) for a in obj_coeffs]) + 1)
        
        #gmodel.write("pre-test.lp")
        
        
        constrs = gmodel.getConstrs()
        ...
        self.atmosts = []
        self.equals = []
        for j, c in  enumerate(constrs):
            #assume ax +b y <=c 
            r = gmodel.getRow(c)
            #coeffs = {r.getVar(i).varname: r.getCoeff(i) for i in range(r.size())}
            extra = 0
            line_top = f"{self.topw} "
            line  = " "
            rhs = -c.getAttr(GRB.Attr.RHS)
            sense = c.getAttr(GRB.Attr.Sense)
            assert(sense in ["=","<"])
            for i in range(r.size()):
                coeff = -r.getCoeff(i) 
                assert(abs(coeff) == 1)
                var = r.getVar(i).varname 
                if (coeff < 0):
                    line  += f"-{var} "
                    extra += -1
                else:
                    line  += f"{var} "
            #print(rhs, extra, sense)

            rhs = rhs - extra
            line  += " 0"
            if (rhs != 1):
                extra = 0
                rhs = int(c.getAttr(GRB.Attr.RHS))
                vars = []
                for i in range(r.size()):
                    coeff = r.getCoeff(i) 
                    assert(abs(coeff) == 1)
                    var = int(r.getVar(i).varname)
                    if (coeff < 0):
                        vars.append(-var)
                        extra += -1
                    else:
                ...
                assert(rhs == 1)
                parse_con(line_top + line)
        #exit()
        
        neg_cost = 0
        for i in range(obj.size()):
            print(obj.getCoeff(i), obj.getVar(i).varname )
            coeff = int(obj.getCoeff(i))
            #print(coeff, self.topw)
            assert(coeff < self.topw)

            var = int(obj.getVar(i).varname)
            line  = f"{coeff} -{var} 0"
            
            if (coeff < 0):
                # this clause has a negative weight
                # it will be processed later
                negs.append(tuple([[-var], -coeff]))
                neg_cost = neg_cost + coeff
            else:
                parse_con(line)
        self.cost =  obj.getConstant() + neg_cost
...
class WCNFPlus(WCNF, object):
    ...
    def __init__(self, from_file=None, from_fp=None, from_string=None, from_gmodel = None, comment_lead=['c']):
        """
            Constructor.
        """
        ...
        super(WCNFPlus, self).__init__(from_file=from_file, from_fp=from_fp,
                from_string=from_string, from_gmodel = from_gmodel, comment_lead=comment_lead)
