...
int Run() {
  ...
  if (absl::GetFlag(FLAGS_use_cp_model)) {
    ...
    Model model;
    parameters.set_max_time_in_seconds(360);
    ...
    parameters.set_cp_model_presolve(0);
    ...
    auto solver = model.Mutable<SatSolver>();
    solver->ResetToLevelZero();
    auto* trail = model.Get<Trail>();


    //std::cout << " trail->CurrentDecisionLevel()  " <<  trail->CurrentDecisionLevel()  << "  trail.Index() " <<  trail->Index()  << "solver->NumVariables() " << solver->NumVariables()  << "solver->NumFixedVariables() " << solver->NumFixedVariables() << std::endl;
  
    std::ofstream output;
    output.open (absl::GetFlag(FLAGS_input) + ".results.txt");
    
    output << "status " << response.status()  << std::endl;
    output << "obj " << response.objective_value()  << std::endl;
    assert(solver->NumFixedVariables() == trail->Index());

    for (int i  = 0; i < solver->NumFixedVariables(); ++i) {
        auto fixed_literal = (*trail)[i];
        const int model_var = mapping->GetProtoVariableFromBooleanVariable(
               fixed_literal.Variable());        
        auto lev = (trail->Info(fixed_literal.Variable())).level;
        auto type = (trail->Info(fixed_literal.Variable())).type;
        auto trail_index = (trail->Info(fixed_literal.Variable())).trail_index;
        ...
        if (fixed_literal.IsPositive()) {
          std::cout << " v  "  << fixed_literal <<  " " <<  model_var    <<" sol " << 1 <<" lev " << lev <<  " type "  << type  << " trail_index " << trail_index << std::endl;
          output << "v" << model_var + 1<< ", " <<    1 << " " << std::endl;      
        } else{
          std::cout << " v  " << fixed_literal <<  " " <<   model_var    <<" sol " << 0 <<" lev " << lev  <<  " type "  << type  << " trail_index " << trail_index << std::endl;
          output << "v " << model_var + 1 << ", " <<    0 << " " << std::endl;      
        }
